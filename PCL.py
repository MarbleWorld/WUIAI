...
# PCL_basic_app.py
# Run:
#   pip install streamlit rasterio pyproj numpy folium streamlit-folium matplotlib requests
#   streamlit run PCL_basic_app.py
...
def download_tif_from_url(url):
    """
    Download the remote GeoTIFF once per URL and cache the local temp path.
    """
...
st.set_page_config(page_title="PCL Basic Map-Click", layout="wide")
st.title("PCL Basic Map-Click")
st.caption("Workflow: 1) Load GeoTIFF from GitHub URL → 2) See PCL overlay → 3) Click a point → 4) Type question → 5) RUN")
...
tif_url = st.text_input(
    ...
    help="Use the RAW GitHub URL to the .tif file."
)
...
if load_btn or (st.session_state.loaded_tif_path is None and tif_url.strip()):
    try:
        ...
        st.session_state.loaded_tif_url = tif_url
