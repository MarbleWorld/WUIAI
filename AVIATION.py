...
# # if not GIS_OK:
# #     st.warning(
# #         "Map rendering dependencies are missing in this environment. "
# #         "The app will still run and show tables + answers. "
# #         f"Import error: {GIS_ERR}"
# #     )
...
# #     if missing:
# #         raise RuntimeError(
# #             f"Masterlist missing columns: {missing}. Found columns: {list(df.columns)}"
# #         )
...
# #     if not api_key:
# #         raise RuntimeError(
# #             "OPENAI_API_KEY is not set. Add [openai] api_key to Streamlit secrets "
...
# #     if response.status_code == 401:
# #         raise RuntimeError("OpenSky states request returned 401. The access token may be invalid.")

# #     if response.status_code == 429:
# #         raise RuntimeError("OpenSky states request was rate limited. Try again later.")

# #     if response.status_code >= 400:
# #         raise RuntimeError(
# #             f"OpenSky states endpoint returned {response.status_code}: {response.text[:800]}"
# #         )
...
# #     c = str(callsign or "").strip().lower()

# #     if any(k in t for k in ["heli", "helic", "rotor", "rotary", "helitack"]):
# #         return True

# #     if any(k in c for k in ["heli", "helo"]):
# #         return True
...
# #     df["is_airborne"] = ((df["on_ground"] != True) | (df["alt_m"].fillna(0) > 30)).astype(bool)

# #     airborne = df[df["is_airborne"]].copy()
...
# #     airborne["is_heli"] = [
# #         classify_airframe_is_heli(t, cs)
# #         for t, cs in zip(airborne.get("Type", ""), airborne.get("callsign", ""))
# #     ]

# #     helis = airborne[airborne["is_heli"]].copy()

# #     sample_cols = [
# #         "Agency", "TailNumber", "icao24", "callsign", "Type",
# #         "latitude", "longitude", "alt_m", "velocity", "true_track", "on_ground"
# #     ]

# #     existing_cols = [c for c in sample_cols if c in airborne.columns]
# #     sample = airborne[existing_cols].sort_values(
# #         [c for c in ["Agency", "TailNumber"] if c in existing_cols]
# #     ).head(25).copy()
...
# #         "agencies_airborne": airborne["Agency"].value_counts(dropna=False).to_dict(),
# #         "helicopters_airborne_total": int(len(helis)),
# #         "helicopters_by_agency": helis["Agency"].value_counts(dropna=False).to_dict() if not helis.empty else {},
# #         "sample_airborne": sample.to_dict(orient="records"),
# #     }
...
# #         geometry=[
# #             Point(xy)
# #             for xy in zip(d["longitude"].astype(float), d["latitude"].astype(float))
# #         ],
# #         crs="EPSG:4326",
# #     )
...
# #             0.5,
# #             0.5,
# #             "No matching aircraft in snapshot",
# #             ha="center",
# #             va="center",
# #             transform=ax.transAxes,
# #             fontsize=14,
...
# #         st.error(f"OpenSky fetch failed: {e}")
# #         st.info(
# #             "This is usually caused by OpenSky auth timeout, temporary outage, rate limiting, "
# #             "or blocked outbound network access from Streamlit Cloud. The app did not crash; "
# #             "try rerunning in a minute."
# #         )
# #         st.stop()
...
# #     if agencies_filter:
# #         matched = matched[
# #             matched["Agency"].astype(str).str.upper().isin(
# #                 [a.upper() for a in agencies_filter]
# #             )
# #         ].copy()
...
# #         with c2:
# #             airborne = matched[
# #                 (matched["on_ground"] != True)
# #                 | (matched["alt_m"].fillna(0) > 30)
# #             ].copy()
...
# #         preferred_cols = [
# #             "TailNumber", "Agency", "Type", "callsign",
# #             "longitude", "latitude", "on_ground",
# #             "alt_ft", "speed_kt", "true_track", "icao24"
# #         ]
...
# #         st.markdown("**Matched aircraft table**")
# #         st.dataframe(
# #             display_df[existing_cols].sort_values(
# #                 [c for c in ["Agency", "TailNumber"] if c in existing_cols]
# #             ),
# #             use_container_width=True,
# #             hide_index=True,
...
# #                 fig_for_llm = plot_snapshot_basemap(matched, title=title, bbox=bbox)
# #                 map_url = fig_to_data_url(fig_for_llm)
# #         else:
# #             fig = plot_points(matched)
# #             st.pyplot(fig, clear_figure=True)
# #     except Exception as e:
# #         st.warning(f"Map rendering failed, but the table and answer still work: {e}")
# #         fig = plot_points(matched)
# #         st.pyplot(fig, clear_figure=True)
...
# #         try:
# #             with st.spinner("Generating answer..."):
# #                 answer, cost = ask_snapshot_question(
# #                     summary,
# #                     question=question.strip(),
# #                     map_data_url=map_url,
# #                 )
...
# #             if est is not None:
# #                 st.caption(
# #                     f"OpenAI usage: model={cost.get('model')} | "
# #                     f"prompt={cost.get('prompt_tokens')} | "
# #                     f"completion={cost.get('completion_tokens')} | "
# #                     f"total={cost.get('total_tokens')} | "
# #                     f"est=${est:.6f}"
# #                 )
# #             else:
# #                 st.caption(
...
# #                     f"prompt={cost.get('prompt_tokens')} | "
# #                     f"completion={cost.get('completion_tokens')} | "
# #                     f"total={cost.get('total_tokens')} | est=N/A"
# #                 )
...
# # def fetch_states_anonymous(bbox=None) -> dict:
# #     params = {}
...
# #         msg = "OpenSky anonymous rate limit hit."
# #         if retry_after:
...
# #     max_len = max(len(row) for row in states)
# #     use_cols = cols[:max_len]

# #     df = pd.DataFrame(states, columns=use_cols)
...
# #     df["icao24"] = df["icao24"].apply(normalize_icao24)
# #     df["callsign"] = df["callsign"].astype(str).str.strip().replace({"None": "", "nan": ""})

# #     numeric_cols = [
# #         "longitude", "latitude", "baro_altitude", "geo_altitude",
# #         "velocity", "true_track", "vertical_rate", "category"
# #     ]
...
# # def classify_airframe_is_heli(type_str: str, callsign: str = "", category=None) -> bool:
# #     t = str(type_str or "").strip().lower()
# #     c = str(callsign or "").strip().lower()

# #     if pd.notna(category) and int(category) == 8:
# #         return True
...
# #     airborne["is_heli"] = [
# #         classify_airframe_is_heli(t, cs, cat)
# #         for t, cs, cat in zip(
# #             airborne.get("Type", ""),
# #             airborne.get("callsign", ""),
# #             airborne.get("category", pd.Series([None] * len(airborne))),
# #         )
# #     ]
...
# #     sample_cols = [
# #         "Agency", "TailNumber", "icao24", "callsign", "Type",
# #         "latitude", "longitude", "alt_m", "velocity", "true_track",
# #         "vertical_rate", "on_ground", "category"
# #     ]
...
# #     try:
# #         with st.spinner("Fetching current OpenSky states anonymously..."):
# #             data = fetch_states_anonymous(bbox=bbox)

# #     except Exception as e:
# #         st.error(f"OpenSky fetch failed: {e}")
# #         st.info(
# #             "The anonymous OpenSky request failed. Since your Jupyter test worked, "
# #             "this may be Streamlit Cloud rate limiting, a temporary OpenSky outage, "
# #             "or blocked outbound access from the deployed environment."
# #         )
# #         st.stop()
...
# #             "TailNumber", "Agency", "Type", "callsign",
# #             "longitude", "latitude", "on_ground",
# #             "alt_ft", "speed_kt", "true_track", "vertical_rate",
# #             "category", "icao24"
# #         ]
...
# st.title("OpenSky Streamlit Network Debug")
...
# st.subheader("DNS test")
# try:
#     t0 = time.time()
#     ip = socket.gethostbyname(host)
#     st.success(f"DNS OK: {host} -> {ip} in {time.time() - t0:.2f}s")
# except Exception as e:
#     st.error(f"DNS failed: {type(e).__name__}: {e}")

# st.subheader("Raw socket test")
# try:
#     t0 = time.time()
#     s = socket.create_connection((host, 443), timeout=30)
#     s.close()
#     st.success(f"Socket OK in {time.time() - t0:.2f}s")
# except Exception as e:
#     st.error(f"Socket failed: {type(e).__name__}: {e}")

# st.subheader("Requests test")
# try:
#     t0 = time.time()
#     r = requests.get(
#         url,
#         params=params,
#         headers={"User-Agent": "opensky-streamlit-debug/1.0"},
#         timeout=(60, 90),
#     )
#     st.write("Status:", r.status_code)
#     st.write("Seconds:", round(time.time() - t0, 2))
...
#         data = r.json()
#         st.success(f"States returned: {len(data.get('states') or [])}")

# except requests.exceptions.ConnectTimeout as e:
#     st.error(f"CONNECT TIMEOUT: Streamlit Cloud could not connect to OpenSky. {e}")
# except requests.exceptions.ReadTimeout as e:
#     st.error(f"READ TIMEOUT: Connected, but OpenSky did not respond fast enough. {e}")
# except Exception as e:
...
st.title("LLM-Powered Wildfire Aviation Dashboard")
st.caption("Current aircraft locations from an anonymous OpenSky snapshot, matched to your masterlist")
st.divider()
...
def fetch_states_anonymous(bbox=None) -> dict:
    ...
    try:
        response = requests.get(
            ...
            params=params,
            headers={
                "User-Agent": "Mozilla/5.0 opensky-streamlit-wildfire-dashboard/1.0",
        ...
        raise RuntimeError(
            "OpenSky connection timed out before the server responded. "
            "This means the deployed Streamlit server had trouble opening the HTTPS connection. "
            f"Original error: {e}"
        )
    except requests.exceptions.ReadTimeout as e:
        raise RuntimeError(
            "OpenSky connection opened, but the response took too long. "
            f"Original error: {e}"
        )
    except requests.exceptions.ConnectionError as e:
        raise RuntimeError(
            "OpenSky connection failed. This is usually DNS, routing, firewall, TLS, or cloud networking. "
            f"Original error: {e}"
        )
    except requests.exceptions.RequestException as e:
        raise RuntimeError(f"OpenSky request failed: {type(e).__name__}: {e}")
    ...
    if response.status_code == 429:
        ...
        if retry_after:
            msg += f" Retry after {retry_after} seconds."
        if remaining:
            msg += f" Remaining credits: {remaining}."
        raise RuntimeError(msg)

    if response.status_code >= 400:
        raise RuntimeError(
            f"OpenSky states endpoint returned HTTP {response.status_code}: {response.text[:1000]}"
        )

    try:
        return response.json()
    except Exception:
        raise RuntimeError(f"OpenSky returned non-JSON response: {response.text[:1000]}")
...
if go:
    ...
    try:
        ...
        st.info(
            "Your local Jupyter test proved OpenSky works from your computer. "
            "If this error only happens after deployment, the problem is the deployed Streamlit environment's "
            "network path to OpenSky, not your OpenSky URL or masterlist."
        )

        with st.expander("Show quick Streamlit network debug code"):
            st.code(
                '''
import time
import socket
...
try:
    ...
    r = requests.get(
        ...
        params=params,
        headers={
            "User-Agent": "Mozilla/5.0 opensky-streamlit-debug/1.0",
            "Accept": "application/json",
            "Connection": "close",
        },
        timeout=(60, 120),
